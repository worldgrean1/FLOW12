...
export default function PremiumInteractiveDemo({
  ...
  const handleWordNodeClick = () => {
    if (wordActive && hiddenTextVisible && !isNavigating) {
      ...
      transitionOverlay.className = "fixed inset-0 bg-slate-900 z-[100] pointer-events-none"
      transitionOverlay.style.opacity = "0"
      document.body.appendChild(transitionOverlay)

      // Animate the transition overlay
      const fadeIn = transitionOverlay.animate([{ opacity: 0 }, { opacity: 0.7 }], {
        duration: 300,
        fill: "forwards",
        easing: "ease-in-out",
      })

      // When fade in completes, navigate and then fade out
      fadeIn.onfinish = () => {
        ...
        setTimeout(() => {
          const fadeOut = transitionOverlay.animate([{ opacity: 0.7 }, { opacity: 0 }], {
            duration: 300,
            fill: "forwards",
            easing: "ease-in-out",
          })
          ...
          fadeOut.onfinish = () => {
            // Remove the overlay and reset navigation state
  ...
  return (
    <section className="h-screen text-white flex flex-col overflow-hidden relative">
      <div className="flex-1 flex items-center justify-center px-4 sm:px-2 py-8">
          <motion.div
